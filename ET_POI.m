...
% load data from file
load('ETdata_POI_input.mat')

% extract sensor-specific and overlapping date-specific ETc
selectedsensor = 'Bill_Upper';
sensoridx = find(strcmpi(Tsensor.site,selectedsensor));
sensordates = Tsensor.local_time(sensoridx); % all dates for the daily ETc (selected sensor)
...
ETc = Tsensor.ETc(sensoridx(min(sensoridx_common)):sensoridx(max(sensoridx_common)));
...
% save
save("ET_interpolated.mat","ETmaps_daily","flightday","ETc")
